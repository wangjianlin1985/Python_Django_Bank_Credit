...
class FrontAddView(BaseView):  # 前台客户添加
    ...
    def post(self, request):
        user_name = request.POST.get('userInfo.user_name') # 判断客户编号是否存在
        if self.primaryKeyExist(user_name):
            return JsonResponse({'success': False, 'message': '客户编号已经存在'})

        userInfo = UserInfo() # 新建一个客户对象然后获取参数
        userInfo.user_name = user_name
        userInfo.password = request.POST.get('userInfo.password')
        userInfo.name = request.POST.get('userInfo.name')
        userInfo.xydj = request.POST.get('userInfo.xydj')
        try:
        ...
        userInfo.yhjf = float(request.POST.get('userInfo.yhjf'))
        userInfo.regTime = str(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
        userInfo.save() # 保存客户信息到数据库
        return JsonResponse({'success': True, 'message': '保存成功'})


class FrontModifyView(BaseView):  # 前台修改客户
    def get(self, request, user_name):
        context = {'user_name': user_name}
        return render(request, 'UserInfo/userInfo_frontModify.html', context)

class FrontSelfModifyView(BaseView):  # 前台修改客户
    def get(self, request):
        return render(request, 'UserInfo/userInfo_frontModify.html')


class FrontListView(BaseView):  # 前台客户查询列表
    def get(self, request):
    ...
    def handle(self, request):
        ...
        xydj = self.getStrParam(request, 'xydj')
        ...
        if xydj != '':
            userInfos = userInfos.filter(xydj__contains=xydj)
        if telephone != '':
...
class FrontShowView(View):  # 前台显示客户详情页
    def get(self, request, user_name):
        # 查询需要显示的客户对象
...
class ListAllView(View): # 前台查询所有客户
    def get(self,request):
        userInfos = UserInfo.objects.all()
        userInfoList = []
...
class UpdateView(BaseView):  # Ajax方式客户更新
    def get(self, request, user_name):
        # GET方式请求查询客户对象并返回客户json格式
        userInfo = UserInfo.objects.get(user_name=user_name)
        return JsonResponse(userInfo.getJsonObj())

    def post(self, request, user_name):
        # POST方式提交客户修改信息更新到数据库
        userInfo = UserInfo.objects.get(user_name=user_name)
        userInfo.password = request.POST.get('userInfo.password')
        userInfo.name = request.POST.get('userInfo.name')
        userInfo.xydj = request.POST.get('userInfo.xydj')
...
class BackModifyView(BaseView):  # 后台更新客户
    def get(self, request, user_name):
        context = {'user_name': user_name}
        return render(request, 'UserInfo/userInfo_modify.html', context)


class ListView(BaseView):  # 后台客户列表
    def get(self, request):
        # 使用模板
...
class DeletesView(BaseView):  # 删除客户信息
    def get(self, request):
        return self.handle(request)
...
class OutToExcelView(BaseView):  # 导出客户信息到excel并下载
    def get(self, request):
        # 收集查询参数
        user_name = self.getStrParam(request, 'user_name')
        name = self.getStrParam(request, 'name')
