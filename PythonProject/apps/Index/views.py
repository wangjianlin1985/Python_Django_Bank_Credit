...
class FrontLoginView(View):
    ...
    def post(self,request):
        ...
        try:
            userInfo = UserInfo.objects.get(user_name = username, password = password)
...
class LoginView(View):
    ...
    def post(self,request):
        ...
        if identify == "admin":
            ...
            try:
                employee = Employee.objects.get(employeeNo=username, password=password)
                request.session['username'] = username
                data = {'msg': '登录成功', 'success': True}
...
class MainEmployeeView(View):
    # 后台主界面
    def get(self,request):
        return render(request, 'main_employee.html')
